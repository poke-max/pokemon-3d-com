...
import { PlaygroundScene } from '../phaser/PlaygroundScene'
...
import {
  ...
  STATUS_LABELS,
  STAT_LABELS,
  type PokemonSpeciesMeta,
  type StatId,
} from '../data/speciesMeta'
...
type HpSnapshot = { current: number; max: number }
...
const buildInitialGameState = (): GameState => {
  const teams: Record<PokemonSlot, PokemonInstance[]> = {
    p1: [],
    p2: [],
  }
  ;(['p1', 'p2'] as PokemonSlot[]).forEach((slot) => {
    const selections = AVAILABLE_SELECTIONS_BY_SLOT[slot] ?? []
    teams[slot] = selections.map((selection) => {
      const speciesMeta = SPECIES_META[selection.id]
      const baseMaxHp = selection.maxHp ?? speciesMeta?.hp.max ?? 0
      return {
        uid: generateUid(),
        ...
        nickname: selection.nickname || speciesMeta?.nickname || selection.id,
        level: selection.level ?? speciesMeta?.level ?? 50,
        maxHp: baseMaxHp,
        currentHp: baseMaxHp,
        boosts: {},
        status: null,
  ...
  return {
    teams,
    active: {
      p1: teams.p1[0]?.uid ?? null,
      p2: teams.p2[0]?.uid ?? null,
    },
  }
}
...
const createHpSnapshot = (speciesId: PokemonSpecies | null | undefined): HpSnapshot => {
  if (!speciesId) return { current: 0, max: 0 }
  const meta = SPECIES_META[speciesId]
  if (!meta) return { current: 0, max: 0 }
  return { current: meta.hp.current, max: meta.hp.max }
}
...
> = {
  ...
  takingDamage: { label: 'Recibiendo daño', description: 'El Pokemon esta recibiendo daño.', color: '#f97316' },
...
export function PhaserCanvas() {
  ...
  const [slotSelections, setSlotSelections] = useState<Record<PokemonSlot, PokemonSelection | null>>({
    p1: DEFAULT_SLOT_SELECTION.p1,
    p2: DEFAULT_SLOT_SELECTION.p2,
  })
  ...
  const [slotHealth, setSlotHealth] = useState<Record<PokemonSlot, HpSnapshot>>({
    p1: createHpSnapshot(DEFAULT_SLOT_SELECTION.p1.id),
    p2: createHpSnapshot(DEFAULT_SLOT_SELECTION.p2.id),
  })
  const [slotStages, setSlotStages] = useState<Record<PokemonSlot, Partial<Record<StatId, number>>>>({
    p1: getInitialStages(DEFAULT_SLOT_SELECTION.p1.id),
    p2: getInitialStages(DEFAULT_SLOT_SELECTION.p2.id),
  })
  ...
  const previousSlotSpeciesRef = useRef(slotSpecies)
  ...
  useEffect(() => {
    const updates: Partial<Record<PokemonSlot, HpSnapshot>> = {}
    const stageUpdates: Partial<Record<PokemonSlot, Partial<Record<StatId, number>>>> = {}
    ALL_POKEMON_SLOTS.forEach((slot) => {
      if (previousSlotSpeciesRef.current[slot] !== slotSpecies[slot]) {
        updates[slot] = createHpSnapshot(slotSpecies[slot])
        stageUpdates[slot] = getInitialStages(slotSpecies[slot])
      }
    })
    previousSlotSpeciesRef.current = slotSpecies
    if (!Object.keys(updates).length) return
    setSlotHealth((current) => {
      const next = { ...current }
      ;(Object.keys(updates) as PokemonSlot[]).forEach((slot) => {
        next[slot] = updates[slot]!
      })
      return next
    })
    if (Object.keys(stageUpdates).length) {
      setSlotStages((current) => {
        const next = { ...current }
        ;(Object.keys(stageUpdates) as PokemonSlot[]).forEach((slot) => {
          next[slot] = stageUpdates[slot] ?? {}
        })
        return next
      })
    }
...
function getInitialStages(_speciesId: PokemonSpecies | null | undefined) {
  // No aplicar boosts iniciales; todos los stats empiezan neutrales.
  return {}
}
...
const renderStatStages = (meta?: PokemonSpeciesMeta) => {
  ...
  const resolveSlotMeta = (slot: PokemonSlot) => {
    ...
    const baseMeta = SPECIES_META[instance.species]
    if (!baseMeta) return undefined
    const currentHp = Math.round(instance.currentHp)
    const maxHp = instance.maxHp || baseMeta.hp.max
    const stagesState = instance.boosts
    return {
      ...baseMeta,
      nickname: instance.nickname || baseMeta.nickname,
      level: instance.level ?? baseMeta.level,
      hp: {
        current: currentHp,
        max: maxHp,
      },
 ...
 /*      if (isDamage && !options?.skipDamageAnimation) {
      ...
      const nickname =
        ...
        (speciesId && SPECIES_META[speciesId]?.nickname) ||
        slot.toUpperCase()
      appendLogEntry(
  ...
  const triggerSwapPokemon = (
    ...
    setGameState((current) => {
      const team = current.teams[slot] ?? []
      const options = AVAILABLE_SELECTIONS_BY_SLOT[slot] ?? []
    ...
    const nickname = SPECIES_META[selection.id]?.nickname ?? selection.id
    appendLogEntry(
  ...
  // Centro único para orquestar las acciones de batalla (ataques, debilitar, swap, etc.)
  type BattleAction =
  ...
  const handleFaintState = (
    ...
    setBattleFeed((current) => [...current, `¡${nickname} se debilitó!`].slice(-3))
  ...
  const playScheduledSequence = async () => {
    ...
    try {
      ...
      setBattleFeed((current) => [...current, '¡Es supereficaz!'].slice(-3))
  ...
  const handleBattleProtocolCommand = (command: string) => {
    ...
    if (action === 'move') {
      ...
      setBattleFeed((current) => [...current, `¡${attackerName} usó ${moveName}!`].slice(-3))
      return
    }
    ...
    if (action === '-unboost' || action === '-boost') {
      ...
      if (actualDelta === 0) {
        appendLogEntry(`El stat ${STAT_LABELS[parsed.stat]} ya está en el límite.`)
        return
      }
  ...
  const handleAttackCommand = (payload: {
    ...
    const currentState = battleStates[payload.slot]
    if (currentState === 'debilitado') {
      handleLogEvent(`El slot ${payload.slot.toUpperCase()} esta debilitado y no puede atacar.`)
      return
    }
    if (currentState === 'contactAttack' || currentState === 'rangedAttack' || currentState === 'statusAttack') {
      handleLogEvent(`El slot ${payload.slot.toUpperCase()} ya esta ejecutando un movimiento.`)
      return
    }
    if (payload.animationType === 'contact') {
      performBattleAction({
        type: 'contact',
        slot: payload.slot,
        moveName: payload.moveName,
      })
    } else if (payload.animationType === 'ranged') {
      performBattleAction({
        type: 'ranged',
        slot: payload.slot,
        moveName: payload.moveName,
      ...
      performBattleAction({
        ...
        slot: payload.slot,
        moveName: payload.moveName,
        selfTarget: payload.moveTarget === 'self',
      })
    }
    const attackerName =
      findInstanceByUid(gameState, payload.slot, gameState.active[payload.slot])?.nickname ??
      payload.slot.toUpperCase()
    setBattleFeed((current) =>
      [...current, `¡${attackerName} usó ${payload.moveName || 'un movimiento'}!`].slice(-3)
    )
  }
      ...
      <div ref={containerRef} className="phaser-container" style={containerStyle}>
        ...
        {!hasEmptySlot && (
          <AttackPanel
            ...
            teamOptions={AVAILABLE_SELECTIONS_BY_SLOT}
            ...
            hidden={ALL_POKEMON_SLOTS.some((slot) => isExecutingMove(slot))}
          />
        )}
      </div>
      ...
      {hasEmptySlot ? (
        <div className="control-log-row">
          <div className="action-panel action-panel--empty-slot">
            ...
            <PokemonSwapPanel
              teams={AVAILABLE_SELECTIONS_BY_SLOT}
              activeSelection={slotSelections}
