...
const tripleBattleModulePath = path.resolve(vendorRoot, 'apps/triple-battle-demo/TripleBattleComponent.ts');
const randomSetsPath = path.resolve(vendorDataPath, 'random-battles/gen9/sets.json');
const randomSets = require(randomSetsPath) as Record<string, {
	level?: number;
	sets?: Array<{ movepool?: string[]; abilities?: string[] }>;
}>;
...
function buildCatalog(): CatalogPayload {
	...
	for (const mon of dex.species.all()) {
		...
		const preferredMoves = (randomMovePool.length ? randomMovePool : (mon.randomBattleMoves || mon.randomDoubleBattleMoves || []))
			.map((moveid: string) => {
				const move = dex.moves.get(moveid);
...
const createManualSession = (teams: any) => {
	...
	const initialPlayerSnapshot = summarizeManualRoster(teams.p1.roster, 'p1');
	pushLog(`|smartai|Tus Pokémon: ${JSON.stringify(initialPlayerSnapshot)}`);
	const initialCpuSnapshot = summarizeManualRoster(teams.p2.roster, 'p2');
	...
	const startScript = [
		...
		`>player p1 ${JSON.stringify({ name: 'Jugador 1', team: Teams.pack(teams.p1.roster) })}`,
		`>player p2 ${JSON.stringify({ name: 'CPU', team: Teams.pack(teams.p2.roster) })}`,
	].join('\n');
	...
	return {
		...
		configureServer(server) {
			...
			server.middlewares.use('/api/simulate', async (req, res) => {
				if (req.method !== 'POST') {
					res.statusCode = 405;
					res.end('Método no permitido, usá POST.');
					return;
				}
				try {
			...
			server.middlewares.use('/api/manual/start', async (req, res) => {
				...
				try {
					const payload = await readRequestBody(req);
					const session = createManualSession(payload.teams);
...
export default defineConfig(({ command }) => ({
	plugins: [
		react(),
		command === 'serve' ? tripleBattleApiPlugin() : null,
